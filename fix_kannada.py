...
# Find the start and end of the Kannada section
kn_start = None
kn_end = None
...
for i, line in enumerate(lines):
    if 'kn: {' in line:
        kn_start = i
    if kn_start is not None and i > kn_start and '},' in line and 'ar:' in lines[i+1] if i+1 < len(lines) else False:
        kn_end = i
        break

if kn_start is None:
    print("Could not find Kannada section start")
    exit(1)

if kn_end is None:
    print("Could not find Kannada section end")
    exit(1)

print(f"Found Kannada section from line {kn_start+1} to {kn_end+1}")
...
new_kannada = """    kn: {
        ...
        explore_banner_desc: "ನಮ್ಮ ಪ್ಯಾಣ, ಧ್ಯೇಯ ಮತ್ತು ದೃಷ್ಟಿಕೋನವನ್ನು ಅನ್ವೇಷಿಸಿ",
...
new_lines = lines[:kn_start] + [new_kannada] + lines[kn_end+1:]

# Write back with UTF-8 encoding
with codecs.open('script.js', 'w', encoding='utf-8') as f:
    f.writelines(new_lines)

print(f"Successfully replaced Kannada section (lines {kn_start+1} to {kn_end+1})")
print("File saved with UTF-8 encoding")
print("\nPlease test the Kannada language in your browser now!")
